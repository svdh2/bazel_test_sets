...
- **Orchestrator Main**: The `burn-in` subcommand triggers state transitions; invokes `process_results` after test execution when `status_file` is configured, driving state transitions from orchestrator results
