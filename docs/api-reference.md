...
| `--mode` | string | `diagnostic` | Execution mode: `diagnostic`, `detection`, `regression` |
| `--max-parallel` | int | CPU count | Maximum parallel test executions |
| `--max-failures` | int | unlimited | Stop after N failures |
| `--output` | path | - | Path to write YAML report |
| `--status-file` | path | - | Path to `.tests/status` JSON state file |
| `--diff-base` | string | - | Git ref for regression mode diff |
| `--changed-files` | string | - | Comma-separated changed files (regression mode) |
